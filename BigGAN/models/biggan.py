...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        # Set up optimizer
        # If this is an G_EMA copy, no need for an optim, so just return now
        if no_optim:
            return
...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        # Width multiplier
        self.ch = D_ch
        # Use Wide D_lr as in BigGAN and SA-GAN or skinny D_lr as in SN-GAN?
        self.D_wide = D_wide
        # Resolution
        self.resolution = resolution
        # Kernel size
        ...
        # Which convs, batchnorms, and linear layers to use
        # No option to turn off SN in D_lr right now
        if self.D_param == 'SN':
            self.which_conv = functools.partial(
                layers.SNConv2d,
                kernel_size=3,
        ...
        for index in range(len(self.arch['out_channels'])):
            ...
            # If attention on this block, attach it to the end
            if self.arch['attention'][self.arch['resolution'][index]]:
                print('Adding attention layer in D_lr at resolution %d' %
                      self.arch['resolution'][index])
                self.blocks[-1] += [
                    layers.Attention(self.arch['out_channels'][index], self.which_conv)
                ]
        ...
        for module in self.modules():
            if (isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear)
                ...
                if self.init == 'ortho':
                    ...
                    print('Init style not recognized...')
                self.param_count += sum([p.data.nelement() for p in module.parameters()])
        print('Param count for D_lr' 's initialized parameters: %d' % self.param_count)
