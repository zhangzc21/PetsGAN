...
def subsequence(sequential,
    ...
    '''
    assert ((single_layer is None) or (first_layer is last_layer is upto_layer is None))
    ...
    for name, layer in sequential._modules.items():
        ...
        if name == upto_layer:
            ...
            including_children = False
        if including_children:
            included_children[name] = layer if share_weights else (copy.deepcopy(layer))
        if name == last_layer:
            last_layer = None
            including_children = False
