...
def dgp_update_config(config):
    config['G_activation'] = activation_dict[config['G_nl']]  # 激活函数
    config['D_activation'] = activation_dict[config['D_nl']]  # 激活函数
...
def load_weights(G,
    ...
    if D is not None:
        D.load_state_dict(
            torch.load(
                '%s/%s.pth' %
                (weights_root, join_strings('_', ['D_lr', name_suffix])),
                map_location=map_func),
            strict=strict)
...
def name_from_config(config):
    name = '_'.join([
        item for item in [
            ...
            'D_lr%s' % config['D_param'] if config['D_param'] != 'SN' else None,
